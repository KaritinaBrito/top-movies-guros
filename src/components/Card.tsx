...
export const Card = ({id, title, poster, vote_average, description, genre_ids}: CardProps) => {
    const src = `https://image.tmdb.org/t/p/w500${poster}`;

    return(
        <main className="flex flex-col gap-5" >
            <section className="flex flex-col rounded-md shadow-lg  gap-2 cursor-pointer lg:flex-row-reverse lg:gap-5">
                <section className="min-w-[355px] min-h-[275px] relative lg:w-[335px] lg:max-w-[355px] lg:max-h-[355px]">
                    <Image loader={() => src} src={src} alt={title} fill className="rounded-t-md object-cover lg:rounded-r-md lg:rounded-tl-none"/>                
                ...
                <section className="px-2 lg:flex lg:flex-col lg:w-full">
                    ...
                    <div className="flex mb-4 justify-end gap-2 lg:justify-start lg:mt-40 ">
                        {genre_ids?.map(item => (
                            <p className="text-guros-blue500 text-[10px] leading-4 rounded-full border-solid border border-guros-blue500 py-1 px-3">{item}</p>
                        )) }
                    </div>
                </section>  
            </section>            
